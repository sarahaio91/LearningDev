...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="kopgeg">
    ...
    <xsl:choose>
      ...
      <xsl:when test="current()/ancestor::uitspraaktekst">
        <xsl:choose>
          <xsl:when test="string-length($titel) and string-length($kopnr)">
            <div class="kopgegp1">
              <span class="kopnrp">
  ...
  <xsl:template match="a | essentie | uitspraakred| uitspraakredromp | annotatieromp | annotatiekop">
    <div class="hftekst">
      <xsl:apply-templates />
    </div>
  </xsl:template>
